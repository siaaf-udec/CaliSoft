<div class="modal fade" id="ayuda">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Ayuda Modulo Proyecto Grado</h5>
      </div>
      <div class="modal-body">
        <div class="panel-group" id="accordion">
          <div class="panel panel-default">
              <div class="panel-heading" style="background-color:#0E3D38;">
                <h4 class="panel-title" >
                  <a data-toggle="collapse" data-parent="#accordion" href="#collapse2">
                    <label class="font-white">Funciones Modulo Proyectos Grado</label></a>
                </h4>
              </div>
            <div id="collapse2" class="panel-collapse collapse">
              <div class="panel-body">
                <ul>
                  <li>Visualizar datos de proyectos registrados en la plataforma CaliSoft.</li>
                  <li>Si su proyecto es en grupo, puede enviar la solicitud a otro compañero, previamente el debe estar registrado.</li>
                  <li>Podra enviar la propuesta del proyecto si esta seguro de los datos que a registrado.</li>
                  <li>Podra editar los datos del proyecto si hay algun dato incorrecto.</li>
                  <li>Podra eliminar la propuesta del proyecto si no esta seguro de realizarlo.</li>
                  <li>Una vez el proyecto sea aceptado podran vizualizar los evaluadores asignados.</li>
                  <li>Cuando esten todos los documentos cargados podran realizar la evaluación del proyecto</li>
                </ul>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="modal-footer" slot="footer">
        <button type="button" class="btn dark btn-outline" data-dismiss="modal">Cerrar</button>
      </div>
    </div>
  </div>
</div>
